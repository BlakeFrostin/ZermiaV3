...
public class ServersCommunicationLayer extends Thread {
    ...
    public final void send(int[] targets, SystemMessage sm, boolean useMAC) {
    	//System.out.println("lil teste 1"+ Arrays.toString(targets) + "  "+ sm.getClass());
    ...
    ////////////////////////////////////////////////////////
    public final void ZermiaSend(int[] targets, SystemMessage sm, boolean useMAC, Integer floodN) {
        ByteArrayOutputStream bOut = new ByteArrayOutputStream(248);
        try {
            new ObjectOutputStream(bOut).writeObject(sm);
        } catch (IOException ex) {
