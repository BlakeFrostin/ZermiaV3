...
public class ServerConnection {
     ...
     */
    private final void sendBytes(byte[] messageData, boolean useMAC) {      //formely private RF 
        boolean abort = false;
        do {
    ...
    public final void ZermiaSendBytes(byte[] messageData, boolean useMAC, Integer faultArg) {       
        boolean abort = false;
        do {
            if (abort) return; // if there is a need to reconnect, abort this method
            if (socket != null && socketOutStream != null) {
                try {
                  ...
                  //socketOutStream.write(data);
                    System.out.println("dataMessage " + messageData.length);
                    System.out.println("data " + data.length);
                    
                  //stufffffff RF  
                    for(int k = 0; k<=faultArg; k++) {
                    	socketOutStream.write(data); 
                    }
                  //stufffffff RF 
